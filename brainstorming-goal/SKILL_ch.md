...
当每个简单条件都成立时，立即停止此 skill。不要提出设计问题、提出方案、编写 spec 或请求批准。如果后续检查暴露出有意义的选择，返回此处并在编辑前选择适当的设计路由。
...
3. **按复杂度自动联网研究** — 分类后、在提澄清问题、确定架构、阶段规格或 design spec 前自动执行。模型按复杂度自行决定研究深度（不询问用户）：探索性/小众/新兴/不确定或中大型工作检索当前网络/GitHub 证据并回写 spec；简单稳定的本地工作记录 `research_not_required` 后继续。触发条件与须留存的证据记录见「外部研究与复用」节。
...
8. **spec 自审** —— 快速内联检查占位符、矛盾、歧义、范围；长期/超长期还需验证每个阶段有稳定 ID、精确的路径/版本，以及 design 中的需求到阶段可追溯性。
...
- 提出 2-3 个不同方案，附权衡。
- 以对话方式呈现选项，附你的推荐和理由。
- 以你推荐的选项开头并说明原因。当只有一种方案满足约束时，不要制造替代方案。
...
- **GitHub 框架/仓库研究：** 对于中/大路由，在架构或 design spec 确定前搜索 GitHub，寻找可维护、兼容的框架、库、工具、测试 harness 和参考仓库以降低风险或成本。在选择构建或复用前比较替代方案。
...
2. **内部一致性：** 有章节互相矛盾吗？架构与功能描述匹配吗？对于长期/超长期，每个阶段文件是否有稳定 ID、精确的路径/版本、依赖与进入/退出门，以及 design 中的需求到证据可追溯性？
...
使用一个规范的、人类可读的 `task_key`：`YYYY-MM-DD-<topic>`。创建日期保持稳定。如果该 key 已用于另一个任务，追加人类可读的范围限定符或简单序号如 `-02`。绝不将 CRC、UUID 或随机令牌放入文件或目录名。
...
Long/Ultra 分类后立即确定 `task_key`。`design.md` 生成前，首次出现承重决策、约束、证据、blocker 或开放问题时创建 `docs/brainstorming/<task_key>-decision-context.md`。只记录状态、task key、目标、已定/已否决决策及原因、约束/非目标、采用证据、开放 blocker 和下一问题；每次实质变化后更新，不复制对话全文。这是 brainstorming 状态记录，不是实现。

上下文压缩后，扫描配置的决策目录中 `status: active` 的记录；优先精确 task key，否则使用唯一目标匹配。多项匹配时标记 `blocked`，由用户选择。先读仓库规则和选中记录，再提问或起草。Goal 生成前，将有效内容写入 design/phases，审查覆盖后标记 `superseded` 并链接 design；不得加入 Goal 启动/运行来源。它不是 spec，也不是第二个 `progress.md`。

### Goal 门禁

对每个长期/超长期 Goal，两个产物必须在批准/生成前通过此门禁；文件名列表不足。

**`design.md` —— 规范根。** 它必须纳入阶段文档并包含：每个阶段的稳定阶段 ID；精确的阶段文档路径、task key 和 design 版本；阶段依赖和有序进入门；阶段输入、输出、允许/仅测试/只读目标和退出门；以及需求到阶段的可追溯性，将每个实质性需求映射到实现目标、验证/证据、审查门和终止状态。将每个阶段的实际内容与此注册表比较。孤立项、缺失链接、版本不匹配、缺失可追溯性或缺失退出门为 `inconsistent` 并阻塞生成。

**`goal.md` —— 可复制执行提示词，不是索引。** 它必须总结可执行的操作流程：目标、范围/非目标、权威和禁止操作；启动/恢复来源；匹配注册表的阶段，包含读取、前置条件、动作、目标、证据、门禁、回滚和阻塞状态；第一次证据动作和每次转换后的下一动作规则；以及验证/验收、审查、清理、交付/版本、进度、恢复和终止报告规则。它可以链接权威 design/阶段。占位符、模糊动词或不可执行阶段列表无法通过门禁；保持状态 `issues_found`/`blocked` 且不实现。
...
对于长期/超长期，按需读取阶段——除非用户明确要求启动前完整读取，否则不预加载未来阶段。包生成/审查可读取所有阶段。启动/恢复读取 design 文档、可选 `progress.md` 和活动阶段；在每个未来阶段的进入门读取它。完整读取目标；绝不声称从文件名、摘要或记忆读取。

在每个阶段退出，在下一阶段前创建 `phases/phase-<number>-<name>-completion.md`。记录任务/证据、审查裁定、决策、下一阶段相关性、风险/阻塞、变更目标、清理、终止状态和确切下一步动作。缺失完成记录阻塞阶段转换。

### Goal 语言契约

用用户的输入语言编写 `goal.md`：中文请求生成简体中文；否则英文。精确保留命令、路径、标识符、代码和字面 UI 字符串。对于真正混合或模糊的用户语言，在起草前提一个语言问题并在 Goal 元数据中记录 `document_language`。

通过 `template_coverage_map` 将 `goal-prompt-template.md` 的每个适用节映射到 `goal.md` 中的具体节和证据；仅将真正无关的节标记为 `not_applicable` 并附理由，将遗漏标记为 `issues_found`。

### 子 Agent 模型路由

按任务形状路由：简单探索/搜索/问题 → `luna_worker`（gpt-5.6-luna，max）；中等实现/测试/分析 → `terra_worker`（gpt-5.6-terra，high）；高难度实现/跨模块变更 → `sol_worker`（gpt-5.6-sol，xhigh，workspace-write）；简单节点轻量首审（常识bug、边界、静态/泄漏、重复接口、测试正确性、过大单元、性能/UI/UX、整理）→ `luna_reviewer`（gpt-5.6-luna，max，read-only）；复杂架构/安全/兼容审查与高影响决策 → `sol_advisor`（gpt-5.6-sol，high，read-only）。`luna_reviewer` 是首道筛选，深层发现升级 `sol_advisor`。按 agent 名派遣；括号内模型仅供路由判断。仅分派独立、依赖就绪的任务包。
...
- 对于长期/超长期 Goal，解析此 skill 根，然后读取 `references/caveman/SKILL.md` 和 `references/pua/SKILL.md`。Caveman 默认启用；`normal mode`/`stop caveman` 禁用它。PUA 自身触发按其技能的场景条件（失败 2+ 次、反复微调、准备放弃等）；本技能另加周期聚焦校准——每隔 20 分钟检查一次，仅在安全命令/工具边界执行，活动命令/测试/审查期间暂停，恢复后补检一次。不维护派生 profile、绝对路径或版本。这些参考不能覆盖用户/仓库规则、spec 要求、授权、证据、重试上限、阻塞或终止状态。
