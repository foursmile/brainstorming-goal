...
- pre-spec recovery: when a decision-context record exists, every active decision is folded into reviewed design/phases, content coverage is checked, and the record is marked `superseded`; it is absent from Goal runtime sources;
- phase integration: when reviewing a design, verify every phase file has a stable ID, exact path/version in the design registry, matching task key/version, dependency and entry/exit gates, and requirement-to-proof traceability; review the actual phase content and reject a filename-only phase list;
...
- review/evidence: required independent gates, artifact paths/versions, content-delta rulings, invalidation rules, and exceptions are traceable;
